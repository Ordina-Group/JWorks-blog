...
The keynote room could accommodate a very large crowd.
...
The talk starts with explaining why Keptn was created and why other tools like ArgoCD (because App health degraded).
...
Keptn goes further if an application that before responded to request in say 200 ms,
but your new version of the application response in 600 ms, would you consider it a healthy application?
No, and neither does Keptn if you configure it right.

It can use OpenTelemetry traces and prometheus metrics (but not limited to)
in combination with standard metadata labels to identify links and versions.
You must specify the following labels: app.kubernetes.io/name; app.kubernetes.io/version and app.kubernetes.io/part-of.
Using that meta-data Keptn can observe your deployments for your business application and identify bad deployments.

You can check acceptable response times by specifying SLI,
SLO and SLA's this has the benefit of allowing you to implement a day 2 operation.

With the help of Keptn you can monitor if an application was deployed, 
check if everything is healthy
and then trigger promotion using a hook with all the information of the deployment to you CI/CD tool

Keptn adds a subset of the DORA observability metrics and traces to your kubernetes events,
making it possible to ingest then in any tool that supports OpenTelemetry

Argo loves Keptn and are exploring ways to better integrate Keptn into Argo
and are asking the community how to better integrate it.

Conclusion: Keptn is a tool that uses OpenTelemetry adds observability to your deployments,
validate SLIs/SLOs and promote them into other environments.

Check out their [documentation](https://keptn.sh/stable/docs/) for use cases and how to set it up.
