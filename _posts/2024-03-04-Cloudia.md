...
During the development of this project we explored a few AI tools to assist us with various tasks. We explored the possibilities for coding in a variety of scripting and programming languages to see whether these tools help us with greater productivity, increased development speed and better bug searching. These tools can generate and autocomplete code. Furthermore, they claim to help with code refactoring, are often integrated with IDEs and helps diagnose and fix bugs.

## CodeWhisperer

Amazon CodeWhisperer is an open-source development tool that facilitates efficient collaboration and communication among software development teams. It aims to improve the overall development proces by providing a platform for sharing knowledge, code, and best practices, ultimately leading to better code quality and faster development cycles. CodeWhisperer enables developers to work more cohesively and make informed decisions during the software development lifecycle. It fosters an environment where code reviews, feedback, and discussions can happen seamlessly within the development team.
...
In our project we explored it mainly for Java which has miexed results. Amazon CodeWhisperer does generate code, but is notably slow and may not even produce the desired output. For example, an attempt to generate attributes for a specific class may resulted in unusual attributes being generated. Additionally, manual intervention is needed since code generation for endpoints can be somewhat inconsistent.

In conclusion, you cannot rely fully on CodeWhisperer for Java development, but it may augmenting your existing code to some degree.
...
Our experience with GitHub Copilot has been anything but positive.

Suggestions take quite long to appear and Copilot fails to understand the context of our code beyond classes.
For instance, when creating a 'Person' class with a name and birthdate and asking Copilot to write tests for this class.
The result were tests for the name and birthdate (as expected) but also tests to see if the phone number is in the correct format or testing the gender of the person. Both of these are clearly unneeded since our class lacks these attributes.

The only thing it excelled at was writing small functions, such as calculating the sum of two numbers or checking if a number is prime.

In conclusion, it currently takes more time to review and correct suggestions than doing it manually.

## ChatGPT
We also explored ChatGPT during our project as an AI tool. It has proven to be useful in some cases but not always.
Oftentimes, we provide code as input and chatGPT can help us locate issues, modify the code to different formats (such as a different date time format that needs to be adjusted on many lines of code manually otherwise), give suggestions when we have bugs and so on.

Sometimes it's a wolf in sheeps clothing since suggestions and ideas proposed may simply be wrong or outdated or incompatible whilst ChatGPT insists that it is correct (and a simple google search points out to us that it was actually wrong for some reason). However, it provides useful in many ways as long as you keep in mind the drawbacks and imperfections. If issues persist, have a look elsewhere in case the tool is outdated or wrongly informated. Oftentimes it helps provide an easy way to modify code or get started with something new.
...
#### AI Conclusion
In conclusion it is still early for many AI tools to help with code development and bug fixing. However, this seems promising for the future and already some tools can augment us in multiple ways.
As of now, ChatGPT is in our opinion the most useful instance to play around with and 
