...
In the second job, the application is dockerized and pushed to ECR. 
...
But to be efficient with our resources we delete them at the end of the workday. 
Since it is possible that someone works in the late evening on the project, we have configured a Telegram notification. 
Before destroying the resources, a message/question is sent to a telegram channel asking whether it's okay to destroy the resources. 
...
A P1 digital electricity meter is being read and sent to an MQTT topic. We have multiple lambda's that gets triggered when a new message (data) is sent to this topic. The lambda's convert and save this data to the database. 
