...
Now let's take a look how the conversion looks like in Java.

OBIS code data is converted into JSON format with the following steps.

Multiple methods were created to work together in converting OBIS code data to JSON format. The first method is is the process line. This method will be runned on each line of the raw data that it has received. It processes each OBIS code line and calls the mapAndSetValue method to map it to the correct property of our POJO object. which receives OBIS code data and maps it to the correct property of our POJO object (SmartMeterData).
...
The mapAndSetValue methdod will map the data to the correct property of our POJO object.
...
This workflow has two jobs. In the first job, a native image is build and Terraform runs afterwards to create all the resources. 
...
````
In the second job, the application is dockerized and pushed to ECR. See example below.
...
Now we also want to deploy and provision these AWS resources at work hours. This is done using scheduler. 
In the following screenshot an example is shown how to configure the scheduler on the workflow. 
...
We also have deployed our application on AWS App runner which is also configured with Spring Security. The application interacts with the databases. To interact with the AWS App Runner, Api Gateway and Route 53 has been configured. Cognito is also configured on this Api Gateway to have security enabled on the API Gateway itself. So an authenticated user can make requests to the domain hosted on Route 53. These requests are then routed through the AWS API Gateway, which subsequently forwards them to the AWS App Runner for processing.
